...
#   027 makes files read-only for group and inaccessible for others, which some sec>
#   might consider benefitial, especially if Jenkins runs in a box that's used for >
#   Beware that 027 permission would interfere with sudo scripts that run on the ma>
#
#   Note also that the particularly sensitive part of $JENKINS_HOME (such as creden>
#   written without 'others' access. So the umask values only affect job configurat>
#   that sort of things.
#
#   If commented out, the value from the OS is inherited,  which is normally 022 (a>
